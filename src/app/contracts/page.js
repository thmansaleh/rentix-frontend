...
export default function ContractsPage() {
  ...
  const filteredContracts = contracts.filter(contract => {
    const matchesSearch = contract.contract_number.toLowerCase().includes(searchTerm.toLowerCase()) ||
                          contract.reservation_id.toLowerCase().includes(searchTerm.toLowerCase()) ||
                          contract.customer_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
                          contract.vehicle_model.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  const handleDeleteContract = (contractNumber) => {
    setContracts(prev => prev.filter(c => c.contract_number !== contractNumber));
  };
            ...
            >
             ...
             <div className="flex items-center gap-4 mt-2 text-sm text-muted-foreground">
                <div className="flex items-center gap-1">
                  ...
                  <span>{t('active')}: {contracts.filter(c => c.status === 'active').length}</span>
                </div>
                <div className="flex items-center gap-1">
                  <div className="w-2 h-2 bg-green-500 rounded-full"></div>
                  <span>{t('completed')}: {contracts.filter(c => c.status === 'completed').length}</span>
                </div>
                <div className="flex items-center gap-1">
                  <div className="w-2 h-2 bg-red-500 rounded-full"></div>
                  <span>{t('cancelled')}: {contracts.filter(c => c.status === 'cancelled').length}</span>
                </div>
              </div>
          </CardContent>
        </Card>
        ...
        {filteredContracts.length === 0 && (
          <Card className="shadow-md border border-border bg-card backdrop-blur-sm">
            <CardContent className="p-12 text-center">
              ...
              <h3 className="text-xl font-semibold text-foreground mb-2">{t('noContracts')}</h3>
              <p className="text-muted-foreground mb-6">{t('noContractsMessage')}</p>
