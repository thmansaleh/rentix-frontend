...
function MoneyTransactions() {
    ...
    const [transactions, setTransactions] = useState([
        { id: 1, user: 'أحمد', amount: 100, method: t('cash') },
        { id: 2, user: 'سارة', amount: 200, method: t('card') }
    ]);
    ...
    const handleAddTransaction = () => {
        ...
        setTransactions([
            ...transactions,
            { id: transactions.length + 1, user: '', amount: Number(amount), method }
        ]);
        setAmount('');
        setMethod(t('cash'));
        setError('');
    ...
    const handleDeleteFee = () => {
        setAdditionalFees(additionalFees.filter(fee => fee.id !== deleteFeeId));
