...
const AssetModal = ({ 
  ...
  const handleDeleteExistingDocument = (documentId) => {
    setDeleteDocConfirm(documentId)
  }

  const confirmDeleteExistingDocument = async () => {
    const documentId = deleteDocConfirm
  ...
  return (
    <>
      <CustomModal
        isOpen={isOpen}
        onClose={onClose}
        title={isEditMode ? (isArabic ? 'تعديل الأصل' : 'Edit Asset') : (isArabic ? 'إضافة أصل جديد' : 'Add New Asset')}
        size="md"
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              placeholder={isArabic ? 'مثال: سيارة، معدات، عقد إيجار، رخصة تجارية' : 'e.g., Vehicle, Equipment, Lease Agreement, Business License'}
              disabled={isLoading}
            />
          </div>
      ...
      <AlertDialog open={!!deleteDocConfirm} onOpenChange={() => setDeleteDocConfirm(null)}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={confirmDeleteExistingDocument} className="bg-red-500 hover:bg-red-600">
              {isArabic ? 'حذف' : 'Delete'}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
