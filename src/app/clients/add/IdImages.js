...
function IdImages() {
  ...
  const imageSlots = [
    { 
      key: 'idFront', 
      label: t('clients.registrationForm.idAndPassportInfo.idImages.idFrontLabel'),
      icon: CreditCard,
      placeholder: t('clients.registrationForm.idAndPassportInfo.idImages.idFrontPlaceholder')
    },
    { 
      key: 'idBack', 
      label: t('clients.registrationForm.idAndPassportInfo.idImages.idBackLabel'),
      icon: CreditCard,
      placeholder: t('clients.registrationForm.idAndPassportInfo.idImages.idBackPlaceholder')
    },
    { 
      key: 'passportFront', 
      label: t('clients.registrationForm.idAndPassportInfo.idImages.passportFrontLabel'),
      icon: BookOpen,
      placeholder: t('clients.registrationForm.idAndPassportInfo.idImages.passportFrontPlaceholder')
    },
    { 
      key: 'passportBack', 
      label: t('clients.registrationForm.idAndPassportInfo.idImages.passportBackLabel'),
      icon: BookOpen,
      placeholder: t('clients.registrationForm.idAndPassportInfo.idImages.passportBackPlaceholder')
    }
  ]
  ...
  const handleFileSelect = (event, imageType) => {
    ...
    if (file.size > maxFileSize) {
      alert(t('clients.registrationForm.idAndPassportInfo.idImages.fileTooLarge').replace('{fileName}', file.name))
      return
    }

    if (!file.type.startsWith('image/')) {
      alert(t('clients.registrationForm.idAndPassportInfo.idImages.invalidFileType'))
      return
    }
 ...
 <h4 className="text-lg font-semibold mb-4  flex items-center gap-2">
          ...
          {t('clients.registrationForm.idAndPassportInfo.idImages.title')}
        </h4>     
        <div className="flex justify-between items-center mt-4">
          <span className="text-sm text-gray-500">{t('clients.registrationForm.idAndPassportInfo.idImages.uploadCount').replace('{count}', uploadedCount)}</span>
          {uploadedCount > 0 && (
            <button 
              onClick={removeAllImages}
              className="text-red-500 hover:text-red-700 text-sm underline"
            >
              {t('clients.registrationForm.idAndPassportInfo.idImages.deleteAllImages')}
            </button>
          )}
        </div>
      </div>
                ...
                {image ? (
                  ...
                  <div className="flex flex-col items-center justify-center h-full text-gray-500">
                    ...
                    <p className="text-xs text-gray-400 text-center">
                      {t('clients.registrationForm.idAndPassportInfo.idImages.clickToSelect')}<br />
                      {t('clients.registrationForm.idAndPassportInfo.idImages.dragAndDrop')}
                    </p>
                  </div>
                )}
