...
import { getCaseEmployeesDocuments, deleteCaseEmployeeDocument } from '@/app/services/api/cases';
import { toast } from 'react-toastify';

function EmployeeDocuments({ caseId }) {
  ...
  // Fetch employee documents using SWR
  const { 
    ...
    caseId ? `case-employees-documents-${caseId}` : null,
    () => getCaseEmployeesDocuments(caseId),
    {
  ...
  const handleDeleteDocument = async () => {
    ...
    setIsDeleting(true);
    try {
      await deleteCaseEmployeeDocument(caseId, deleteModal.documentId);
      ...
      <Card>
        <CardContent className="p-6">
          <div className="text-red-500 text-center">
            ...
            <p>{isArabic ? 'خطأ في تحميل مستندات الموظفين' : 'Error loading employee documents'}</p>
          </div>
        </CardContent>
      </Card>
    );
        ...
        <CardContent>
          {isLoading ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-muted-foreground">
                {isArabic ? 'لا توجد مستندات للموظفين' : 'No employee documents found'}
              </p>
            </div>
          ) : (
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-4 text-xs text-muted-foreground">
                        <div className="flex items-center gap-1">
                          ...
                          <span>{formatDate(document["created at"])}</span>
                        </div>
