...
import { createAnnualLeave, updateAnnualLeave } from '@/app/services/api/annualLeaves'
...
const AnnualLeaveModal = ({ 
  ...
  onSuccess,
  employeeId,
  annualLeave = null // If provided, we're editing
}) => {
  const { t } = useTranslations()
  const { language } = useLanguage()
  const isArabic = language === 'ar'
  const isEditMode = !!annualLeave
  ...
  useEffect(() => {
    if (isEditMode && annualLeave) {
      setFormData({
        employee_id: annualLeave.employee_id,
        date: annualLeave.date ? new Date(annualLeave.date) : null,
        from_date: annualLeave.from_date ? new Date(annualLeave.from_date) : null,
        to_date: annualLeave.to_date ? new Date(annualLeave.to_date) : null,
        total_days: annualLeave.total_days.toString(),
        remaining_days: annualLeave.remaining_days.toString(),
        leave_type: annualLeave.leave_type
      })
    } else {
      setFormData({
        employee_id: employeeId,
  ...
  }, [annualLeave, isEditMode, employeeId])
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      let response
      if (isEditMode) {
        response = await updateAnnualLeave(annualLeave.id, submitData)
      } else {
        response = await createAnnualLeave(submitData)
      }
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[600px]">
        <DialogHeader>
          <DialogTitle>
            {isArabic 
              ? (isEditMode ? 'تعديل إجازة سنوية' : 'إضافة إجازة سنوية جديدة')
              : (isEditMode ? 'Edit Annual Leave' : 'Add New Annual Leave')
            }
          </DialogTitle>
        </DialogHeader>
