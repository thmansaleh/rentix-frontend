...
import { getAnnualLeaves, deleteAnnualLeave } from '@/app/services/api/annualLeaves'
...
function AnnualLeave({ employeeId }) {
  ...
  const [selectedAnnualLeave, setSelectedAnnualLeave] = useState(null)

  // SWR fetcher function
  const fetcher = async (url) => {
    const response = await getAnnualLeaves(employeeId)
    if (response.success) {
  ...
  const { data: annualLeaves, error, isLoading } = useSWR(
    employeeId ? `/api/annual-leaves?employee_id=${employeeId}` : null,
    fetcher,
    {
  ...
  if (error) {
    toast.error(error.message || (isArabic ? 'حدث خطأ أثناء تحميل البيانات' : 'Error loading annual leaves'))
  }

  const handleAddAnnualLeave = () => {
    setSelectedAnnualLeave(null)
    setIsModalOpen(true)
  }

  const handleEditAnnualLeave = (annualLeave) => {
    setSelectedAnnualLeave(annualLeave)
    setIsModalOpen(true)
  }
  ...
  return (
    <div className="space-y-4">
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              <CardTitle>{isArabic ? 'الإجازات السنوية' : 'Annual Leaves'}</CardTitle>
              <CardDescription>
                {isArabic ? 'إدارة الإجازات السنوية للموظف' : 'Manage employee annual leaves'}
              </CardDescription>
            </div>
            <Button onClick={handleAddAnnualLeave}>
                      ...
                      <TableCell className="text-right">
                        <div className="flex justify-end gap-2">
                          <Button
                            variant="ghost"
                            size="icon"
                            onClick={() => handleEditAnnualLeave(leave)}
                          >
                          ...
                          <Button
                            variant="ghost"
                            size="icon"
                            onClick={() => handleDeleteAnnualLeave(leave.id)}
                          >
      ...
      <AnnualLeaveModal
        ...
        annualLeave={selectedAnnualLeave}
      />
    </div>
  )
}
