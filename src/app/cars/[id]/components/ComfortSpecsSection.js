...
import { addComfortSpec, removeComfortSpec } from '@/redux/slices/editCarSlice'
import ButtonComfortSend from './send-buttons/ButtonComfortSend'

export default function ComfortSpecsSection() {
  const t = useTranslations();
  const {isRTL} = useLanguage();
  const dispatch = useDispatch();
  const specs = useSelector(state => state.editCar?.comfortSpecs || []);
