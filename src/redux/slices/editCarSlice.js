...
// Async thunk to fetch car details from local api JSON and return the payload
export const fetchCarDetails = createAsyncThunk(
  "editCar/fetchDetails",
  async (carId, thunkAPI) => {
    // currently the demo JSON is at /api/car-details.json
    const res = await fetch(`/api/car-details.json`);
    if (!res.ok) throw new Error("Failed to fetch car details");
    const data = await res.json();
    // some endpoints wrap the payload in `data`, prefer that if present
...
const editCarSlice = createSlice({
  ...
  reducers: {
    ...
    'plate_source', 'plate_number', 'brand_id', 'model', 'year',
    ...
    loadCarData: (state, action) => {
      ...
      Object.keys(carData).forEach(key => {
        if (key === 'plate_source_id' && carData[key] !== undefined) {
          // map incoming id to plate_source for form selects
          state.plate_source = carData[key];
          state.plate_source_id = carData[key];
          return;
        }
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchCarDetails.fulfilled, (state, action) => {
        ...
        const carData = action.payload || {};
        // mirror existing loadCarData behaviour: only copy known keys
        Object.keys(carData).forEach((key) => {
          if (key === 'plate_source_id' && carData[key] !== undefined) {
            state.plate_source = carData[key];
            state.plate_source_id = carData[key];
